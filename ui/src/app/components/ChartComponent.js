	...
	return (
		<div className="w-full">
			<nav className='w-full p-3 flex align-middle justify-center bg-white '>
				<span className="mx-3 text-sm font-medium text-gray-900 dark:text-black">Show detail panel</span>
				...
				<span className="mx-3 text-sm font-medium text-gray-900 dark:text-black">Chart type</span>
			...
			<div className={`${className} relative text-black`}>
				{isCurrentDataVisible && currentData && (
					<div className={`hidden sm:block absolute top-0 left-0 right-0 mx-auto z-40 p-3 bg-gray-400 bg-opacity-50 rounded-md w-1/6`}>
						...
						<div className='flex justify-between'><div>Close</div> <div>{currentData['close'].toFixed(2)}</div></div>
						<div className='flex justify-between'><div>Volume</div> <div>{currentData['volume']}</div></div>
						<div className='flex justify-between'><div>% Change</div> <div>{currentData['change'].toFixed(2)} %</div></div>
					</div>
				)}
